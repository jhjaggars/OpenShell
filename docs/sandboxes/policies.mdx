...
When GPU devices are present in the sandbox container, OpenShell also adds the GPU filesystem baseline. This includes the injected NVIDIA or WSL2 device nodes as read-write paths. CUDA initialization also writes `/proc/<pid>/task/<tid>/comm`; OpenShell handles that with a GPU-only Landlock `WriteFile` exception for procfs, so `/proc` can remain in `read_only`. If a custom policy explicitly keeps an injected GPU device node in `read_only`, sandbox startup fails and tells you to move that device path to `read_write`.
