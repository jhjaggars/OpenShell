...
For the `sidecar` or `proxy-pod` topology, skip this grant and instead set `sandboxServiceAccount.openshift.nonrootSCC=true` when installing the chart, which binds the built-in `nonroot-v2` SCC. See [Topology](/kubernetes/topology) for the per-topology privilege model.
