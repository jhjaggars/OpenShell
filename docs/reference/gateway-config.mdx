...
Each sandbox runs as a lightweight Linux VM on macOS with Apple Silicon via Apple's [`container`](https://github.com/apple/container) CLI and the macOS Virtualization framework. Requires macOS 26 and the `container` system service running (`container system start`).
